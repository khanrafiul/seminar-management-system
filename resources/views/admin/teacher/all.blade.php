  ...
  <div class="col-md-12">
    <div class="card">
      <div class="card-header">
        <div class="row">
          <div class="col-md-8">
            <h3 class="card-title text-secondary table-card-title-one"><i class="fa fa-cubes"></i> All Teacher</h3>
          </div>
          <div class="card-title col-md-4 text-right table-card-title-two">
            <a href="{{url('admin/teacher/create')}}" class="btn btn-sm btn-info card_head_btn"><i class="fa fa-th"></i> Add Teacher</a>
          </div>
        ...
        <table id="datatable" class="table table-striped table-bordered table-dark" cellspacing="0" width="100%">
          ...
          <tbody>
            <tr>
              ...
              <td>
                @if($value->image==true)
                  <img src="{{ asset('storage/teacher') }}/{{ $value->image }}" height="30px" width="30px" />
                @else
              ...
              <td>{{$value->courseName->name}}</td>
              <td>
                @php
                  $id=$value->id;
                  $total=App\Seminar::where('status',1)->where('teacher_id',$id)-> count();
                  echo $total;
                @endphp
              </td>
              <td>

                <a class="teacherapprove teacherreject btn btn-success btn-link btn-icon btn-sm like" data-teacherstatus="{{ url('admin/teacher/status/'.$value->id) }}" data-toggle="modal" 
                @if($value->status==true)
                ...
                <a href="{{ url('admin/teacher/'.$value->id) }}" class="btn btn-info btn-link btn-icon btn-sm like"><i class="fa fa-eye"></i></a>

                <a href="{{ url('admin/teacher/'.$value->id.'/edit') }}" class="btn btn-warning btn-link btn-icon btn-sm edit"><i class="fa fa-edit"></i></a>

                <a class="teacherdelete btn btn-danger btn-link btn-icon btn-sm remove"data-teacherdelete="{{url('admin/teacher/'.$value->id)}}" data-toggle="modal" data-target="#exampleModalDelete"><i class="fa fa-trash-alt"></i></a>
...
<div class="modal fade" id="exampleModalApprove" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <form action="" method="post" id="teacherapprove">
        @csrf
        @method('put')
        ...
        <div class="modal-body">
            <p class="text-danger font-weight-bold text-center">Do you really want to approve teacher request?</p>
        </div>
...
<div class="modal fade" id="exampleModalReject" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <form action="" method="post" id="teacherreject">
        @csrf
        @method('put')
        <div class="modal-header">
          <h5 class="modal-title font-weight-bold" id="exampleModalLabel"> <p class="text-danger"> Are you sure! </h5>
        ...
        <div class="modal-body">
            <p class="text-danger font-weight-bold text-center">Do you really want to reject teacher request?</p>
        </div>
        <div class="modal-footer">
          <button type="submit" class="btn btn-info" data-dismiss="modal">No, Cancel</button>
